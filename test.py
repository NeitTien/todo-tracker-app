...
def create_gui():
    ...
    sidebar.grid(row=0, column=0, sticky="n", padx=15, pady=20)
    ...
    calendar_view.columnconfigure(0, weight=1)
    calendar_header = tk.Frame(calendar_view)
    calendar_header.grid(row=0, column=0, sticky="ew", pady=(0, 8))
    calendar_header.columnconfigure(1, weight=1)
    calendar_header.columnconfigure(2, weight=1)
    calendar_grid = tk.Frame(calendar_view)
    calendar_grid.grid(row=1, column=0, sticky="ew")
...
if __name__ == "__main__":
    create_gui()
    window.mainloop()  # Exactly one main application event loop, at the end.
